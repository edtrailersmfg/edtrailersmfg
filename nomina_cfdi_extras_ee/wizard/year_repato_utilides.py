...
class XLSUploadWizard(models.TransientModel):
    ...
    total_repartir = fields.Float("Total a repartir")
    ...
    def reparto_utilidades_data(self):
        ...
        work_lines = payslips.mapped('worked_days_line_ids').filtered(lambda x: x.code in ['WORK100', 'VAC', 'FJC', 'SEPT'])
        ...
        for employee in amount.items():
           # _logger.info('empleado %s', employee)
             ...
             contract_id = employee[0].contract_id.id
    ...
    def reparto_utilidades_payslip(self):
        ...
        for employee in amount.items():
            ...
            other_inputs = []
            other_inputs.append((0,0,{'name' :'Reparto utilidades', 'code' : 'PTU', 'contract_id':contract_id, 'amount': total01}))
            worked_days2 = []
