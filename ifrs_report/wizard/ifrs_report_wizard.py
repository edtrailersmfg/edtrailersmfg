...
def get_xls(rec):
    ...
    if rec.columns=='2_columns':
        ...
        ws.write(l, 12, 'Noviembre',style_concept)
        ws.write(l, 13, 'Diciembre',style_concept)
        ws.write(l, 14, 'Ajustes',style_concept)
        l += 1
    ...
    def print_report(self):
        ...
        if self.report_format == 'spreadsheet':
            ...
            if self.columns=='12_columns':
                ...
                html1, html2 = self.env.ref('ifrs_report.ifrs_landscape_pdf_report_action').with_context(context)._render_qweb_html(self.ids)
            else:
                html1, html2 = self.env.ref('ifrs_report.ifrs_portrait_pdf_report_action').with_context(context)._render_qweb_html(self.ids)
            
            
            xls_stream = get_xls(self)            
            self.write({'spreadsheet_file': base64.encodebytes(xls_stream), 'spreadsheet_file_name': self.ifrs_id.name.replace(' ','_') + '.xls'})
