...
#### Librerias Licenciamiento ####

from cryptography.fernet import Fernet

#### FIN - Librerias Licenciamiento ####

####### Validando el No. de Serie #########
...
class ResCompany(models.Model):
    ...
    def validate_app_serial(self):
        if not self.odoo_serial_key:
            return False
        else:
            company_vat = self.partner_id.vat
            if not company_vat:
                return False
            company_vat = company_vat.upper()
            odoo_serial_key = self.odoo_serial_key
            key = '5e8AG3Jc0WNRHjiBtVti5aYJ9mva4zf5jLUk8jkTEUU='
            try:
                rfc_bs = Fernet(key.encode()).decrypt(odoo_serial_key.encode())
                if rfc_bs.decode() == company_vat:
                    return True
                else:
                    return False
            except:
